...
SNR=10.^(SNR_db/10);
No_MntCrl=500;
ns=[100, 200, 300, 400, 500, 600, 700, 800, 900, 1000]; %Number of snapshots of array output that are generated and used
    %in estimating spatial correlation matrix R
for l=1:length(ns)
    Qv=diag([6.0,2.0,0.5,2.5,3.0,1.0,5.5,10.0]);
    suminvQv=sum(1./diag(Qv));
    sigmasq=(M/suminvQv)*SNR*ones(1,2);
    ...
    %*************************************************************************
    %********************* Data Generation ***********************************
    smt=zeros(M,ns(l));% Matrix containing sum of source signals in the output of array
    ss=zeros(scnum,ns(l));% Matrix containing source signals in the origin of coordinates
    ...
    CRB_stoc(:,:,l)=(1/ns(l))*inv(2*real((P*A_tild'*inv(R_tild)*A_tild*P).*...
    ...
    for cnt=1:No_MntCrl
        ...
        if (cors==0)||(scnum==1)
            for ii=1:scnum
                ss(ii,:)=sqrt(sigmasq(ii))*sqrt(1/2)*(randn(1,ns(l))+1i*randn(1,ns(l)));
            ...
            v1=sqrt(1/2)*(randn(1,ns(l))+1i*randn(1,ns(l)));
            v2=sqrt(1/2)*(randn(1,ns(l))+1i*randn(1,ns(l)));
            v3=sqrt(1/2)*(randn(1,ns(l))+1i*randn(1,ns(l)));
        ...
        scm=(ss*ss')/ns(l);% Estimating signal covariance matrix
        nsemt=zeros(M,ns(l));% Matrix of noise in the outputs of array elements
        
        nsemt=sqrt(1/2)*sqrt(Qv)*(randn(M,ns(l))+1i*randn(M,ns(l))); % In this case the additive
        ...
        R=(xmt*xmt')/ns(l);% Estimating output covariance matrix
        ...
        for t=1:length(si)
            ...
            if (t>2 && t<length(si))
                ...
                if (10*log10(S_IMLSE(t-1))>10*log10(S_IMLSE(t-2))&& 10*log10(S_IMLSE(t-1))>10*log10(S_IMLSE(t)) && 10*log10(S_IMLSE(t-1))>0)% 
                    ...
                    degS_IMLSE(cnt4)=-90+(IdxS_IMLSE(cnt4)/20);
                end
                if (10*log10(S_ILSSE(t-1))>10*log10(S_ILSSE(t-2))&& 10*log10(S_ILSSE(t-1))>10*log10(S_ILSSE(t)) && 10*log10(S_ILSSE(t-1))>0)% 
                    ...
                    degS_ILSSE(cnt5)=-90+(IdxS_ILSSE(cnt5)/20);
                end
            end
        end
        ...
        if (length(degsais)~=length(degS_IMLSE))
            sumS_IMLSE=(sum((degsais-[degS_IMLSE degS_IMLSE(end)]).^2)/2)+sumS_IMLSE;
        else
            sumS_IMLSE=(sum((degsais-degS_IMLSE).^2)/2)+sumS_IMLSE;
        end
        if (length(degsais)~=length(degS_ILSSE))
            sumS_ILSSE=(sum((degsais-[degS_ILSSE degS_ILSSE(end)]).^2)/2)+sumS_ILSSE;
        else
            sumS_ILSSE=(sum((degsais-degS_ILSSE).^2)/2)+sumS_ILSSE;
        end
        cnt
    end
...
loglog(ns,CRB_stocf,'Color',[0 1 0.1],'LineWidth',1.25)
hold on
semilogx(ns,RMSES1,'-.sr','LineWidth',1.25)
semilogx(ns,RMSES2,'--*b','LineWidth',1.25)
semilogx(ns,RMSES3,'-.om','LineWidth',1.25)
semilogx(ns,RMSES_IMLSE,'LineStyle','-','Marker','>','Color',[0.9 0.3 0.1],'LineWidth',1.25)
semilogx(ns,RMSES_ILSSE,'LineStyle','-.','Marker','h','Color',[0.1 0.5 0.4],'LineWidth',1.25)
axis([100 1000 10^-1 3])
title({'RMSEs(deg.^{\bullet}) of DOA estimation versus No of Snapshot';'No of Iteration:500, SNR : 3dB, Sourse Degree:[-3 6]'...
      ;'WNPR : 20, Correlation Coefficient=0'})
legend('CRB_{stoc}','Traditional MUSIC','Matrix Completion Method','Proposed Method','IMLSE','ILSSE')
xlabel('No of Snapshot'); ylabel('DOA RMSE(Deg.)');
grid on
toc
