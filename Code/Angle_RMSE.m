...
SNR=10.^(SNR_db/10);
No_MntCrl=500;
tetha2=[6 9 12 15 18 21];
for l=1:length(tetha2)
...
semilogy(tetha2+3,CRB_stocf,'g')
hold on
plot(tetha2+3,RMSES1,'--sr')
plot(tetha2+3,RMSES2,'--*b')
plot(tetha2+3,RMSES3,'-om')
plot(tetha2+3,RMSES_IMLSE,'LineStyle','-','Marker','>','Color',[0.9 0.3 0.1],'LineWidth',1.25)
plot(tetha2+3,RMSES_ILSSE,'LineStyle','-.','Marker','h','Color',[0.1 0.5 0.4],'LineWidth',1.25)
axis([9 24 0 1])
title({'RMSEs(deg.^{\bullet}) of DOA estimation versus Angle Seperation';'No of Iteration:500, SNR=3dB'...
      ;'No of Snapshpts : 500, Correlation Coefficient=0'})
legend('CRB_{stoc}','Traditional MUSIC','Matrix Completion Method','Proposed Method','IMLSE','ILSSE')
xlabel('Angle Seperation'); ylabel('DOA RMSE(Deg.)');
grid on
toc
