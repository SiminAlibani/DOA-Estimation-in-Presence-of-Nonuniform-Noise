...
for l=1:length(SNR)
    ...
    for cnt=1:No_MntCrl
        ...
        for t=1:length(si)
            ...
            if (t>2 && t<length(si))
                if (10*log10(S1(t-1))>10*log10(S1(t-2))&& 10*log10(S1(t-1))>10*log10(S1(t)) && 10*log10(S1(t-1))>0)
                    cnt1=cnt1+1;
        ...
        subplot(2,3,1);plot(si,10*log10(S1));subplot(2,3,2);plot(si,10*log10(S2));subplot(2,3,3);plot(si,10*log10(S3));
        subplot(2,3,4);plot(si,10*log10(S_IMLSE));subplot(2,3,5);plot(si,10*log10(S_ILSSE));
...
hold on
plot(SNR_db,RMSES1,'--sk','LineWidth',1.25)
plot(SNR_db,RMSES2,'--*k','LineWidth',1.25)
plot(SNR_db,RMSES3,'-ok','LineWidth',1.25)
