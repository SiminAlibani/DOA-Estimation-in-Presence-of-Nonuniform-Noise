...
SNR=10.^(SNR_db/10);
No_MntCrl=500;
qv1=[6.0, 2.0, 0.5, 2.5, 3.0, 1.0, 5.5, 10.0];
qv2=[6.0, 2.0, 0.5, 2.5, 3.0, 1.0, 5.5, 15.0];  qv3=[6.0, 2.0, 0.5, 2.5, 3.0, 1.0, 5.5, 20.0];
qv4=[6.0, 2.0, 0.5, 2.5, 3.0, 1.0, 5.5, 25.0];   qv5=[6.0, 2.0, 0.5, 2.5, 3.0, 1.0, 5.5, 30.0];
qv6=[6.0, 2.0, 0.5, 2.5, 3.0, 1.0, 5.5, 35.0];     qv7=[6.0, 2.0, 0.5, 2.5, 3.0, 1.0, 5.5, 40.0];
qv8=[6.0, 2.0, 0.5, 2.5, 3.0, 1.0, 5.5, 45.0];    qv9=[6.0, 2.0, 0.5, 2.5, 3.0, 1.0, 5.5, 50.0];    
Qv=[qv1;qv2;qv3;qv4;qv5;qv6;qv7;qv8;qv9];
wnpr=[20,30,40,50,60,70,80,90,100];
for l=1:length(wnpr)
    
    suminvQv=sum(1./diag(diag(Qv(l,:))));
    ...
    for cnt=1:No_MntCrl
        ...
        nsemt=sqrt(1/2)*sqrt(diag(Qv(l,:)))*(randn(M,ns)+1i*randn(M,ns)); % In this case the additive
...
semilogy(wnpr,CRB_stocf,'g')
hold on
plot(wnpr,RMSES1,'-.sr')
plot(wnpr,RMSES2,'--*b')
plot(wnpr,RMSES3,'-om')
plot(wnpr,RMSES_IMLSE,'LineStyle','-','Marker','>','Color',[0.9 0.3 0.1],'LineWidth',1.25)
plot(wnpr,RMSES_ILSSE,'LineStyle','-.','Marker','h','Color',[0.1 0.5 0.4],'LineWidth',1.25)
axis([20 100 0.2 10])
title({'RMSEs(deg.^{\bullet}) of DOA estimation versus SNR';'No of Iteration:500, SNR=3dB, Sourse Degree:[-3 6]'...
      ;'No of Snapshots : 500, Correlation Coefficient=0'})
legend('CRB_{stoc}','Traditional MUSIC','Matrix Completion Method','Proposed Method','IMLSE','ILSSE')
xlabel('Worst Noise Power Ratio'); ylabel('DOA RMSE(Deg.)');
grid on
toc
