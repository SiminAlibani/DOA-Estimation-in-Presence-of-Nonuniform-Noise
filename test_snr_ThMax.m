...
%         for t=1:length(si)
            tmp1=1i*2*pi*fc*d*(sin(si*pi/180))/c;
            for jj=1:M
                stv(jj,:)= exp((jj-1)*tmp1);%Steering vector of the ii'th source
            end
...
%             S_ILSSE(t)=norm((stv'*Un_LS),2)^(-2);
            S1=1./sqrt(sum(abs(Vn1'*stv).^2));
            [pks_S1,locs_S1] = findpeaks(S1);
            max_pks_S1=max(pks_S1); 
            slc_ind_S1=find(pks_S1>0.5*max_pks_S1); slc_locs_S1=locs_S1(slc_ind_S1); pks_S1_slc=pks_S1(slc_ind_S1);
            [p_S1,Ind_S1]=sort(pks_S1_slc,'descend');
            if (length(degsais)>length(Ind_S1))
                
            else
                loc_S1=sort(slc_locs_S1(Ind_S1(1:2))); deg_S1=-90+loc_S1./100;
            end
            
            S2=1./sqrt(sum(abs(Vn2'*stv).^2));
            [pks_S2,locs_S2] = findpeaks(S2);
            max_pks_S2=max(pks_S2); 
            slc_ind_S2=find(pks_S2>0.5*max_pks_S2); pks_S2_slc=pks_S2(slc_ind_S2);
            [p_S2,Ind_S2]=sort(pks_S2_slc,'descend');
            loc_S2=sort(slc_ind_S2(Ind_S2(1:2))); deg_S2=-90+loc_S2./100;
            
            S3=1./sqrt(sum(abs(Vn3'*stv).^2));
            [pks_S3,locs_S3] = findpeaks(S3);
            max_pks_S3=max(pks_S3); 
            slc_ind_S3=find(pks_S3>0.5*max_pks_S3); pks_S3_slc=pks_S3(slc_ind_S3);
            [p_S3,Ind_S3]=sort(pks_S3_slc,'descend');
            if (length(degsais)>length(Ind_S3))
                loc_S3=sort(slc_ind_S3(Ind_S3(1))); deg_S3=-90+loc_S3./100; degS3
            loc_S3=sort(slc_ind_S3(Ind_S3(1:2))); deg_S3=-90+loc_S3./100;
%             end
%         end
